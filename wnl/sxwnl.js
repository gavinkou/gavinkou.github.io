...
var msc = {
	...
	toHTML: function(fs) {
		...
		s += '距离     ' + (this.mR).toFixed(2) + 'km     ' + (this.sR).toFixed(8) + 'AU' + '\r\n';
		s += '</pre></center></td></tr>';
	...
	nbj: function(jd) {
		...
		if (V[3] != 100 && V[5] != 100) {
			var x = (V[2] - V[4]) * Math.cos((V[3] + V[5]) / 2),
				y = V[3] - V[5];
			this.Vb = (cs_rEarA * Math.sqrt(x * x + y * y)).toFixed(0) + 'km';
		}
	}
};
...
function Lunar() {
	...
	this.yueLiHTML = function(By, Bm, curJD) {
		...
		var ta0 = '<tr><td colspan=7 style="background-color:#FAEBD7;color:#000000">' + c + '</td></tr>';
